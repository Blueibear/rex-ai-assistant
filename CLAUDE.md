...
- `rex.context.source_policy` is the canonical contextual-use/disclosure metadata layer for connected sources. Source availability, contextual eligibility, audience scope, disclosure policy, and mutation authority remain separate decisions. Private sources require a validated owner and may never be silently promoted to household scope. Integrations deliberately connected by a user may default to that owner's contextual use, while uploads and location retain stricter explicit opt-in rules. Policy changes contribute only content-free revision tokens to `rex.context.cache`, so revocation invalidates private cached artifacts without exposing source content, raw user IDs, or secrets. Filter source authority before retrieving or ranking private content and fail closed on ambiguous ownership or scope.
