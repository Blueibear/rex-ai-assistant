...
- US-074 wake-word runtime diagnostics are privacy-safe live evidence, not configuration guesses. The canonical wake listener may emit bounded detector lifecycle/attempt metadata through its optional callback; the GUI bridge may project only active/configured phrase and backend, threshold, fallback state, detector generation, armed state, selected microphone/device index, bounded confidence/RMS/peak/reject reason, and attempt count. Never forward raw audio, transcripts, prompts, user IDs, credentials, filesystem model/embedding paths, or exception text that may contain them. Repetitive rejected attempts must be throttled, while accepted detections and detector lifecycle changes remain immediate. These diagnostics augment the existing voice path and must not reintroduce the obsolete Electron-owned chat worker or bypass the canonical `rex.background` Core/Voice Agent architecture.
