...
def discover_pyttsx3_voices() -> list[Voice]:
    ...
    try:
        ...
        result = []
        for _idx, voice in enumerate(voices):
            ...
            result.append(Voice(id=voice.id, name=voice.name, provider="pyttsx3", language=lang))
...
def discover_edge_tts_voices() -> list[Voice]:
    ...
    try:
        ...
        for voice in voices:
            ...
            result.append(
                Voice(id=name, name=name, provider="edge-tts", language=lang, gender=gender)
            )
...
def discover_xtts_voices(voices_dir: Path | None = None) -> list[Voice]:
...
def discover_all_voices(provider: str | None = None) -> list[Voice]:
...
def get_voice_dropdown_options(provider: str | None = None) -> list[str]:
