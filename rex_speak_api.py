...
def _handle_auth_error(exc: AuthenticationError):  # pragma: no cover
    LOGGER.warning("Authentication failure: %s", exc)
    return jsonify({"error": str(exc)}), 401

@app.errorhandler(TextToSpeechError)
def _handle_tts_error(exc: TextToSpeechError):  # pragma: no cover
...
def speak() -> Response:
    ...
    try:
        xtts.tts_to_file(
            ...
            language=CONFIG.speak_language or "en",
