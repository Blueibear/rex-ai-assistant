...
**Acceptance Criteria:**
- [ ] `rex/autonomy/feedback.py` implements `FeedbackAnalyzer` with `summarize(goal: str, history_store: HistoryStore) -> str`.
- [ ] Summary retrieves the 10 most recent records. Generates a short (≤ 200 words) text paragraph describing: common failure patterns, which tools tend to succeed/fail, any goals similar to current that succeeded before.
- [ ] Summary generated by the LLM (one compact call). If history is empty, returns empty string.
- [ ] `mypy --strict rex/autonomy/feedback.py` passes. Unit test with mocked history and mocked LLM verifies non-empty output. Test passes.
