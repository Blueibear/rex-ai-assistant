...
Normal non-tool OpenClaw requests use the configured `openclaw_gateway_max_retries` as a retry bound: the initial attempt plus at most that many retries, with bounded backoff for connection/timeout and retryable HTTP 429/5xx failures. `/tools/invoke` is stricter because a mutation may already have reached the gateway: connection loss, 5xx, malformed JSON, or a non-object response after dispatch is treated as outcome-unknown and is not automatically retried. Read-only compatibility calls may use safe local fallback; mutations stay in Rex's canonical lifecycle and become `attempted_unverified` until an independent Rex postcondition verifies them. A 403 remains a policy denial and never falls back around policy; a 404 on the read-only compatibility path means that specific tool is absent remotely and may use the existing local read fallback.

Rex now maintains an explicit reconnect/trust-restoration gate rather than treating the next successful HTTP call as recovered authority. An outage disables current remote bindings immediately. Bounded health probes may establish connectivity, but dispatch remains blocked until a fresh authenticated capability resync succeeds; local Rex capabilities remain usable throughout the outage.
...
Rex validates the complete remote inventory before mutation and durably persists the safe snapshot to household runtime state before projecting canonical metadata and executable bindings. Removed tools and failed/stale refreshes disable existing remote bindings instead of leaving them executable, while local Rex capabilities remain available. Snapshot publication is serialized against remote dispatch, so hot/manual refresh cannot race an old closure through a changed schema or removal; a retained handler also rechecks that it is still the current enabled ToolRegistry binding. Request-supplied context cannot choose the privileged OpenClaw session key, and refreshed bindings replace old gateway/config closures.

Remote result text never grants verification authority. A gateway response that says `completed` or `verified` is normalized as remote evidence only; mutating success requires an accepted Rex verifier/postcondition. If a mutation's post-dispatch outcome becomes uncertain, Rex reports it as attempted but unverified rather than silently retrying or claiming success. Reconnect health events expose only bounded state/reason/attempt/delay metadata and exclude tokens, prompts, arguments, private results, and user content.
...
| `rex/workflow_runner.py` | Wrap | Bridged (US-P4-030, US-021) | Workflow execution. WorkflowBridge preserves Rex policy gating. cli.py migrated to bridge (US-P7-010). `WorkflowBridge.register()` now fails closed with `OpenClawConfigError` when `use_openclaw_tools=True` and the executor API is not available; is a no-op when `use_openclaw_tools=False`. |
...
| `rex/openclaw/errors.py` | New (Phase 8) | **Complete** (US-001, US-021) | `OpenClawConnectionError`, `OpenClawAuthError`, `OpenClawAPIError`, `OpenClawConfigError` — all inherit from `AssistantError`. Raised by `OpenClawClient` on network failure, 401, 4xx/5xx after retries, and feature-enabled-but-API-unavailable respectively. |
