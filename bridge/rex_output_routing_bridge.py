...
def _handle_default_media_account(
    payload: Mapping[str, Any],
    *,
    routing: OutputRoutingService,
    media_accounts: MediaAccountStore,
    ...
    saved = routing.save_policy(user_id, updated)
    return {"ok": True, "policy": _policy_to_dict(saved)}, 0


def _handle_test_playback(
    payload: Mapping[str, Any],
    *,
    registry: AudioTargetRegistry,
    user_id: str,
    ...
    if delivered:
        return {"ok": True, "target_id": target_id}, 0
    return {"ok": False, "error": "Test playback could not be verified."}, 1


def _dispatch_output_routing_command(
    command: str,
    payload: Mapping[str, Any],
    *,
    registry: AudioTargetRegistry,
    ...
    if command == "set_default_media_account":
        return _handle_default_media_account(
            payload,
            routing=routing,
            media_accounts=media_accounts,
            user_id=user_id,
    ...
    if command == "test_playback":
        return _handle_test_playback(payload, registry=registry, user_id=user_id)
...
def handle_output_routing_request(
    ...
    *,
    ...
    try:
        ...
        return _dispatch_output_routing_command(
            command,
            payload,
            registry=registry,
            routing=routing,
