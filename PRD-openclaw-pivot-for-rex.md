...
- [x] Dashboard (OpenClaw) works — all 6 callers migrated off dashboard_store and rex.dashboard; gui_app.py _create_flask_app converted to stub routes in iter 92; retirement check flipped to SAFE TO RETIRE
- [x] Messaging/channels work — messaging_service.py + messaging_backends/ retired in iter 91; sms_tool.py converted to stub; 13 test files for retired modules deleted; retention.py inbound SMS cleanup stubbed; test_retirement_check_messaging.py converted to permanent retirement guard
...
- [x] Call path traced: build_voice_loop() → VoiceLoop.run() → VAD record → Whisper STT → assistant.generate_reply() → TTS
- [x] All Rex modules touched documented (6 modules + 9 external libs)
- [x] Key seam identified: `await self._assistant.generate_reply(transcript)` at rex/voice_loop_optimized.py:493 (no voice_mode=True)
...
- [x] DONE — event_bus.py retired; logic relocated to rex/openclaw/event_bus.py; all 8 blockers migrated; event_bridge.py updated; contracts/event_bus.py removed; 111 targeted tests pass
...
- [x] Policy engine gates all tool calls through OpenClaw — tool_router + tool_registry + event_bus all retired (US-P7-008, US-P7-006, US-P7-002); all 8 OPENCLAW-REPLACE modules retired
- [x] No retired modules remain in codebase — all 8 OPENCLAW-REPLACE modules retired: plugin_loader, executor, browser_automation, messaging_service, dashboard, tool_router, tool_registry, event_bus
