...
RecorderCallable: TypeAlias = Callable[[float], Union[Awaitable[AudioArray], AudioArray]]
...
class AsyncMicrophone:
    ...
    async def _record_with_vad(self, max_duration: float) -> AudioArray:
        ...
        audio = np.concatenate(chunks) if chunks else np.zeros(chunk_frames, dtype=np.float32)
