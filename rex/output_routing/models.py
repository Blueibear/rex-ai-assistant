    ...
    def __post_init__(self) -> None:
        for name in (
            ...
            object.__setattr__(
                self,
                name,
                _validate_target_id(getattr(self, name), field_name=name),
            )
        for name in (
            ...
            object.__setattr__(self, name, FallbackMode(getattr(self, name)))
        for name in (
            ...
            object.__setattr__(
                self,
                name,
                _validate_volume(getattr(self, name), field_name=name),
            )

        if not isinstance(self.prefer_media_request_origin, bool):
            raise ValueError("prefer_media_request_origin must be boolean")
        if self.default_media_provider is not None:
            if (
                not isinstance(self.default_media_provider, str)
                or not self.default_media_provider.strip()
                or self.default_media_provider != self.default_media_provider.strip()
            ):
                raise ValueError("default_media_provider must be a non-empty provider ID")
        if self.default_media_account_id is not None:
            if (
                not isinstance(self.default_media_account_id, str)
                or not self.default_media_account_id.strip()
        ...
        object.__setattr__(
            ...
            "rules",
            tuple(
                rule if isinstance(rule, RoutingRule) else RoutingRule(**rule)
                for rule in self.rules
            ),
        )

        for kind in OutputKind:
            mode, fallback_target = self.fallback_for(kind)
