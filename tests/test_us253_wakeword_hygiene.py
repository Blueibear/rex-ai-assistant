...
def test_root_wakeword_utils_removed():
    assert not (REPO_ROOT / "wakeword_utils.py").exists(), (
        "wakeword_utils.py must not exist at repo root; use rex.wakeword_utils"
    )


def test_root_wakeword_listener_removed():
    assert not (REPO_ROOT / "wakeword_listener.py").exists(), (
        "wakeword_listener.py must not exist at repo root; use rex.wakeword.listener"
    )
