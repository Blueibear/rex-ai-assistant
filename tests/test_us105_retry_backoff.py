...
class TestWithRetrySuccess:
    ...
    def test_succeeds_after_n_failures_custom_config(self, mock_sleep: MagicMock) -> None:
        ...
        for actual_call, exp in zip(mock_sleep.call_args_list, expected, strict=True):
...
class TestExponentialBackoff:
    ...
    def test_delay_doubles_each_attempt(self, mock_sleep: MagicMock) -> None:
        ...
        for actual_call, exp in zip(mock_sleep.call_args_list, expected_delays, strict=True):
            assert actual_call[0][0] == pytest.approx(exp, rel=0.15)
