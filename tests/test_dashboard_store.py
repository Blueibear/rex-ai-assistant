...
def test_notifier_dashboard_channel_writes_to_store(tmp_path: Path):
    """Verify _send_to_dashboard writes to the dashboard store."""
    from rex.dashboard_store import set_dashboard_store
    ...
    try:
        notifier = Notifier(storage_path=tmp_path / "notifier")
        ...
        results = store.query_recent()
        assert len(results) == 1
        assert results[0].title == "Dashboard Test"
        assert results[0].user_id == "alice"
    finally:
        set_dashboard_store(None)


def test_notifier_send_with_dashboard_channel(tmp_path: Path):
    """Verify full notifier.send() flow with dashboard channel."""
    from rex.dashboard_store import set_dashboard_store
    from rex.notification import NotificationRequest, Notifier
    ...
    try:
        ...
        results = store.query_recent()
        assert len(results) == 1
        assert results[0].title == "Full Flow Test"
    finally:
        set_dashboard_store(None)
