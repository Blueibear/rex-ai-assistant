...
def test_us252_listed_scripts_not_at_root() -> None:
    ...
    present_at_root = [
        name for name in (*MOVED_SCRIPTS, *DELETED_SCRIPTS) if (ROOT / name).exists()
    ]
    assert (
        present_at_root == []
...
def test_us252_moved_scripts_exist_in_scripts_dir() -> None:
    ...
    missing = [name for name in MOVED_SCRIPTS if not (SCRIPTS_DIR / name).exists()]
    assert missing == [], f"These scripts should exist under scripts/ but do not: {missing}"


def test_us252_new_scripts_exist_in_scripts_dir() -> None:
    """Scripts newly added for scripts/ must be present."""
    missing = [name for name in NEW_IN_SCRIPTS if not (SCRIPTS_DIR / name).exists()]
    assert missing == [], f"These new scripts should exist under scripts/ but do not: {missing}"
...
def test_us252_scripts_readme_documents_moved_scripts() -> None:
    ...
    assert missing_docs == [], f"scripts/README.md is missing documentation for: {missing_docs}"
