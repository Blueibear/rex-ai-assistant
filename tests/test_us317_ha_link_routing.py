...
def test_home_page_ha_link_not_pointing_to_settings_general():
    ...
    src = read_file("gui/src/pages/HomePage.tsx")
    assert 'to="/settings"' not in src or '/settings/home-assistant' in src, (
        "HomePage.tsx must not have a bare /settings link for HA"
    )
    assert '/settings/general' not in src, (
        "HomePage.tsx must not route HA to /settings/general"
    )
...
def test_home_assistant_settings_page_exports_component():
    ...
    assert 'export function HomeAssistantSettingsPage' in src or \
           'export const HomeAssistantSettingsPage' in src or \
