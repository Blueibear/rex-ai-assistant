...
def test_language_model_custom_strategy():
    ...
    model = LanguageModel(AppConfig(llm_provider="dummy", llm_model="xyz", llm_max_tokens=5))
...
def test_language_model_falls_back_when_transformers_missing(monkeypatch):
    """If transformers and torch are missing, it should fall back to echo strategy."""
    monkeypatch.setattr(llm_impl, "AutoTokenizer", None, raising=False)
    monkeypatch.setattr(llm_impl, "AutoModelForCausalLM", None, raising=False)
    monkeypatch.setattr(llm_impl, "hf_pipeline", None, raising=False)
    monkeypatch.setattr(llm_impl, "torch", None, raising=False)

    model = LanguageModel(AppConfig(llm_model="fallback-model", llm_provider="transformers"))
    completion = model.generate("Prompt")
